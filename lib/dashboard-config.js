...
function normalizedStoredConfig(raw) {
  ...
  const bridges = source.bridges && typeof source.bridges === 'object' && !Array.isArray(source.bridges)
    ? source.bridges
    : {};
  const visibleAgents = Array.isArray(source.visibleAgents)
    ? source.visibleAgents.filter((id, index, values) => (
      typeof id === 'string' && AGENT_ID_PATTERN.test(id) && values.indexOf(id) === index
    )).slice(0, 32)
    : DEFAULT_CONFIG.visibleAgents;
  return {
...
function validateConfigPatch(patch) {
  ...
  for (const key of keys) {
    if (key !== 'alertPercent' && key !== 'bridges' && key !== 'visibleAgents') throw new Error(`不允许配置字段：${key}`);
  }
  ...
  if (hasOwn(patch, 'visibleAgents')) {
    if (!Array.isArray(patch.visibleAgents) || patch.visibleAgents.length > 32) {
      throw new Error('显示 Agent 必须是不超过 32 项的数组');
    }
    if (patch.visibleAgents.some((id, index, values) => (
      typeof id !== 'string' || !AGENT_ID_PATTERN.test(id) || values.indexOf(id) !== index
    ))) {
      throw new Error('显示 Agent 包含无效或重复的标识');
    }
  }
}
...
function createDashboardConfig(options = {}) {
  ...
  function getConfig() {
    return {
      ...
      visibleAgents: stored.visibleAgents === null ? null : [...stored.visibleAgents],
    };
  }
  ...
  function update(patch) {
    ...
    stored = {
      ...
      visibleAgents: hasOwn(patch, 'visibleAgents') ? [...patch.visibleAgents] : stored.visibleAgents,
    };
